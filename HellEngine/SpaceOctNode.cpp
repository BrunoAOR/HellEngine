...
SpaceOctNode::SpaceOctNode(float3 minPoint, float3 maxPoint, SpaceOctTree* aQuadTree, unsigned int aDepth)
	: octTree(aQuadTree), depth(aDepth), childrenCount(4)
{
	assert(octTree);
...
bool SpaceOctNode::Insert(ComponentTransform* transform)
{
	if (isLeaf)
	{
...
bool SpaceOctNode::Remove(ComponentTransform* transform)
{
	if (isLeaf)
	{
...
void SpaceOctNode::CheckBucketSize()
{
	if (containedTransforms.size() > octTree->bucketSize && depth < octTree->maxDepth)
	{
		isLeaf = false;
		/* Divide in X and Y */
		...
		float midY = minPoint.y + (maxPoint.y - minPoint.y) / 2;
		float maxY = maxPoint.y;
		float minZ = minPoint.z;
		float midZ = minPoint.z + (maxPoint.z - minPoint.z) / 2;
		float maxZ = maxPoint.z;
		/* Create child nodes */
		nodes[0] = new SpaceOctNode(minPoint, float3(midX, midY, midZ), octTree, depth + 1);
		nodes[1] = new SpaceOctNode(float3(midX, minY, minZ), float3(maxX, midY, midZ), octTree, depth + 1);
		nodes[2] = new SpaceOctNode(float3(minX, minY, midZ), float3(midX, midY, maxZ), octTree, depth + 1);
		nodes[3] = new SpaceOctNode(float3(midX, minY, midZ), float3(maxX, midY, maxZ), octTree, depth + 1); 
		nodes[4] = new SpaceOctNode(float3(minX, midY, minZ), float3(midX, maxY, midZ), octTree, depth + 1);
		nodes[5] = new SpaceOctNode(float3(midX, midY, minZ), float3(maxX, maxY, midZ), octTree, depth + 1);
		nodes[6] = new SpaceOctNode(float3(minX, midY, midZ), float3(midX, maxY, maxZ), octTree, depth + 1);
		nodes[7] = new SpaceOctNode(float3(midX, midY, midZ), maxPoint, octTree, depth + 1);
